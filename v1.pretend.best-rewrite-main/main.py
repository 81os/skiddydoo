    ...
    def __init__(self):
        super().__init__(
            ...
            openapi_tags=[
                ...
                {
                    'name': 'Status',
                    'description': 'Updates the shard status for all pretend network bots.'
                },
                {
                    'name': 'Master',
...
def update_shards_data(data: list):
  data.append(body)
...
async def pretend_status_info(
    body: ShardsModel,
    token: APIKey = Depends(auth_scheme)
):
    """Update the status for the pretend discord robot"""
    if token.role != 'master': 
        raise HTTPException(
            status_code= status.HTTP_403_FORBIDDEN, 
            detail="You do not have access to update status on this API endpoint"
        )

    result = await post_shard_info(bot, api, "/shards/pretend/post")
    return {"data": {result}}
...
async def greed_status_info(
    body: ShardsModelFull,
    token: APIKey = Depends(auth_scheme)
):
    """Update the status for the greed discord robot"""
    if token.role != 'master': 
        raise HTTPException(
            status_code= status.HTTP_403_FORBIDDEN, 
            detail="You do not have access to update status on this API endpoint"
    ...
    shardsData.append(ShardsModelFull(**body.dict()))
    
@app.get(
    "/shards/pretend/get",
    tags=['status'],
    response_model=ShardsModel
)
async def pretend_shards_get(
    body: ShardsModel,
    token: APIKey = Depends(auth_scheme)
):
    """Update the status for the pretend discord robot"""
    ...
    result = await get_shard_info(bot, api, "/shards/pretend/get")
    return {"data": {'shards': result}}
    
@app.get(
  "/shards/greed/get",
  tags=['status']
)
async def get_shards_data(token: APIKey = Depends(auth_scheme)):
  """Get the data stored in shardsData"""
    if token.role != 'master': 
        raise HTTPException(
            status_code= status.HTTP_403_FORBIDDEN, 
            detail="You do not have access to get data from this endpoint"
        )
  return shardsData  # Return the list of shards
